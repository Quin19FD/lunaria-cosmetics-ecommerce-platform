...
import type { FlashDeal } from "@/modules/promotions";
...
interface FlashDealsProps {
  deals: (FlashDeal & { product: Product })[];
}
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
        ...
        {/* Countdown */}
        <div className="bg-brand-50 ring-brand-100 flex items-center gap-2 rounded-full px-4 py-2.5 ring-1">
          <Timer className="text-brand-500 h-4 w-4" />
            ...
            {[pad(countdown.h), pad(countdown.m), pad(countdown.s)].map(
              (v, i) => (
                <span key={i} className="flex items-center">
                  <span className="bg-brand-500 inline-block min-w-[28px] rounded-md px-1.5 py-0.5 text-center text-sm font-bold text-white tabular-nums">
                    {v}
                  </span>
                  {i < 2 && (
                    <span className="text-brand-300 mx-0.5 text-xs font-bold">
                      :
                    </span>
                  )}
                </span>
            ...
            >
              ...
              >
                <Image
                  ...
                  sizes="200px"
                />
                <span className="bg-brand-500 absolute top-2 left-2 rounded-md px-2 py-0.5 text-[10px] font-bold text-white">
                  -{pct}%
                </span>
              </Link>
              ...
              <div className="flex flex-1 flex-col p-3">
                <Link
                  href={`/products/${deal.product.slug}`}
                  className="hover:text-brand-500 line-clamp-2 text-xs font-medium text-neutral-800 sm:text-sm"
                >
                  {deal.product.name}
                </Link>
                ...
                {/* Progress */}
                <div className="mt-2">
                  <div className="bg-brand-50 h-1.5 overflow-hidden rounded-full">
                    <div
                      className={cn(
                        "h-full rounded-full",
                        soldPct > 80 ? "bg-red-400" : "bg-brand-500",
                      )}
                      style={{ width: `${soldPct}%` }}
                    />
                  </div>
